...
class AirCargoProblem(Problem):
    ...
    def actions(self, state: str) -> list:
        ...
        #decoding the current state and adding only the positive clauses
        propos_logic_knowledgebase = PropKB()
        propos_logic_knowledgebase.tell(decode_state(state,self.state_map).pos_sentence())

        for xaction in self.actions_list:
            possible_to_move = True
            for cl in xaction.precond_pos:
                if cl not in propos_logic_knowledgebase.clauses:
                    possible_to_move = False
                    break
            for cl in xaction.precond_neg:
                if cl in propos_logic_knowledgebase.clauses:
                    possible_to_move = False
                    break
            if possible_to_move:
               possible_actions.append(xaction)
    ...
    def result(self, state: str, action: Action):
        ...
        # any fluents (positive or negative) in action should be carried out to
        # the new state
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        executed.
        """
        # TODO implement (see Russell-Norvig Ed-3 10.2.3 or Russell-Norvig Ed-2
        # 11.2)
        count = 0

        #decoding the current state and adding only the positive clauses
        propos_logic_knowledgebase = PropKB()
        propos_logic_knowledgebase.tell(decode_state(node.state,self.state_map).pos_sentence())
